...
from utils.metrics import AverageMeter, accuracy, Evaluator
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.val_dataloader = dataloader.get_dataloader(split="val", shuffle=False)
        self.length_val_dataloader = len(self.val_dataloader)
        self.length_val_data = len(self.val_dataloader.dataset)

        self.model = models.__dict__[self.args.pretrained_model](args=self.args, **{"num_classes": self.args.num_classes, })
        ckpt = torch.load("./savers/Fold-0/sup_con_resnet/experiment_0/checkpoints/Fold-0-sup_con_resnet-best.pth", map_location='cpu')
        self.model.load_state_dict(ckpt["model_state_dict"])
        self.classifier = models.__dict__[self.args.model](args=self.args, **{"num_classes": self.args.num_classes, })

        self.logger.info(f"Let's use {self.args.loss} loss!")
        self.criterion = losses.__dict__[self.args.loss](args=self.args)

        param_groups = [{"params": self.classifier.parameters(), "lr_mult": self.args.lr_mult_task}]
        ...
        self.evaluator = Evaluator(num_classes=self.args.num_classes)
    ...
    def train(self, epoch):
        ...
        for batch_index, (images, labels) in enumerate(tbar):
            ...
            with torch.no_grad():
                outputs_contrastive = self.model.encoder.task_network(images)
            outputs = self.classifier(outputs_contrastive.detach())
            ...
            self.evaluator.add_batch(predictions, targets)
        ...
        train_accuracy = self.evaluator.accuracy()
        train_recall, train_recall_mean = self.evaluator.recall()
        train_kappa = self.evaluator.kappa()

        self.logger.info(f"Trained Epoch: [{epoch + 1}/{self.args.epochs}]"
                         f"\tLoss: {train_loss:.4f}"
                         f"\tAccuracy: {100 * train_accuracy:.2f}%"
                         f"\tRecall_1: {100 * train_recall[0]:.2f}%"
                         f"\tRecall_2: {100 * train_recall[1]:.2f}%"
                         f"\tRecall_3: {100 * train_recall[2]:.2f}%"
                         f"\tRecall Mean: {100 * train_recall_mean:.2f}%"
                         f"\tKappa: {100 * train_kappa:.2f}%")

        if self.args.no_val:
            if train_accuracy > self.best_accuracy:
                is_best = True
                self.best_accuracy = train_accuracy
                self.checkpoint.save_checkpoint(is_best=is_best, best_accuracy=self.best_accuracy, epoch=epoch, model=self.classifier, 
                                                optimizer=self.optimizer, lr_scheduler=self.scheduler)
            else:
                self.logger.info(f"Best Accuracy is still: {self.best_accuracy:.4f}"
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_index, (images, labels) in enumerate(tbar):
                ...
                outputs_contrastive = self.model.encoder.task_network(images)
                outputs = self.classifier(outputs_contrastive)
                ...
                tbar.set_description(f"Testing Epoch: {epoch + 1}"
                                     f"\tnumImages: [{batch_index * self.args.batch_size + labels.size(0)}/{self.length_val_data}]")

            val_loss = losses.avg

            val_accuracy = self.evaluator.accuracy()
            val_recall, val_recall_mean = self.evaluator.recall()
            val_kappa = self.evaluator.kappa()

            self.logger.info(f"Test Epoch: [{epoch + 1}/{self.args.epochs}]"
                             f"\tLoss: {val_loss:.4f}"
                             f"\tAccuracy: {100 * val_accuracy:.2f}%"
                             f"\tRecall_1: {100 * val_recall[0]:.2f}%"
                             f"\tRecall_2: {100 * val_recall[1]:.2f}%"
                             f"\tRecall_3: {100 * val_recall[2]:.2f}%"
                             f"\tRecall Mean: {val_recall_mean:.2f}%"
                             f"\tKappa: {100 * val_kappa:.2f}%")
            ...
            if val_accuracy > self.best_accuracy:
                ...
                self.logger.info(f"Best Accuracy is still: {100 * self.best_accuracy:.2f}%"
                                 f"\tNo checkpoint file will be saved!")
...
def main():
    ...
    parser.add_argument("--pretrained_model", type=str, default="sup_con_resnet", help="model name")
    parser.add_argument("--model", type=str, default="linear_classifier", help="model name")
    ...
    parser.add_argument("--temperature", type=float, default=1.0, help="temperature for saliency map")
    ...
    parser.add_argument("--loss", type=str, default="CELoss", choices=["CELoss", "FocalCELoss", "LabelSmoothCELoss", "DSCL"],
    ...
    parser.add_argument("--lr_milestones", type=list, default=[30, 60, 80], help="Learning rate milestones (default: [25, 55, 75])")
    ...
    parser.add_argument("--experiment", type=int, default=4, help="experiment name")
    ...
    parser.add_argument("--no_val", type=bool, default=False, help="validation during training")
    # Create the parser
    ...
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        ...
        trainer.logger.info(f"=> Epoch: {epoch + 1}"
                            ...
                            f"\tBest Accuracy: {100 * trainer.best_accuracy:.2f}%")
